...
to go
     ...
     search-homeRange             ;; adults and young adults search for a suitable, occupyable homeRange
...
to init_landscape
     ...
     ifelse Landscape = "Germany" [ file-open "../data/landscape_6.8ha.txt" ] [ file-open "../data/landscape_27.5ha.txt" ]    ;; the landscape is imported
...
;; testing size calculations / only information
    ...
    output-print (word "Home range UM: " precision ((pi * (homeRangeRadiusUM * 10) ^ 2 ) / 10000 ) 0 " ha")
    output-print (word "Home range GER: " precision ((pi * (homeRangeRadiusGER * 10) ^ 2 ) / 10000 ) 0 " ha")
    output-print (word " ")
...
to init_cultivation
     cultivate                                                                   ;; the number of crops in the landscape is counted
end
...
to establish-homeRange                                                                                             ;; juveniles procedure: they become young adults and search for homeRange
     ...
     let i 0
     let found false
     let searchPatches patches in-radius 150 with [(numberOwners < maximumOwners) and (suitability >= thresholdSuitability)]   ;; hares search for suitable patches
     ifelse any? searchPatches
     [
...
to cultivate
     ...
     if (Landscape = "Uckermark" and Scenario = "Crop richness")
     [
     ...
     if (Landscape = "Germany" and Scenario = "Crop richness")
     [
...
to calculate-suitability
     ...
     [
         ...
         if numberOwners > 0
         [
             set suitability suitability - ((numberOwners - 1) * suitabilityReduction)                       ;; if the cell belongs to more than one homeRange, the suitability is reduced by 0.02 for each owner
...
to search-homeRange
     ...
     while [i < 3] [
        ...
        [
            ...
            set searchPatches searchPatches with [(numberOwners < maximumOwners) and (suitability >= thresholdSuitability)]     ;; hares search for suitable patches
        ...
        set i i + 1
        set subset matures with [(suithomeRange < thresholdSuitability) or (any? homeRange with [numberOwners > maximumOverlap])]
     ]
     ...
     ask patches with [numberOwners > maximumOverlap]                                                                        ;; if there are still too many hares within the homeRange, it dies
     [
...
to reproduce
     ...
     [
         hatch offspring                                                  ;; females get 12-15 offspring each year
         [
             set status "juvenile"
             set color yellow
             set size 10
...
View
View
"fields" "crops" "habitat suitability" "territories" "OFF"
2
...
PENS
"Females" 1.0 0 -5298144 true "" "plot count turtles with [status = \"female\"]"
"Males" 1.0 0 -14070903 true "" "plot count turtles with [status = \"male\"]"
"Juveniles" 1.0 0 -1184463 true "" "plot count turtles with [status = \"juvenile\"]"
...
481
Wheat
precision (100 * (count patches with [crop = \"wheat\"]) / 160000) 1
17
1
11
...
531
Rape
precision (100 * (count patches with [crop = \"rape\"]) / 160000) 1
17
1
11
...
581
Maize
precision (100 * (count patches with [crop = \"maize\"]) / 160000) 1
17
1
11
...
631
Barley
precision (100 * (count patches with [crop = \"barley\"]) / 160000) 1
17
1
11
...
681
Grassland
precision (100 * (count patches with [crop = \"grassland\"]) / 160000) 1
17
1
11
...
731
Pasture
precision (100 * (count patches with [crop = \"pasture\"]) / 160000) 1
17
1
11
...
781
Beets
precision (100 * (count patches with [crop = \"beets\"]) / 160000) 1
17
1
11
...
481
Alfalfa
precision (100 * (count patches with [crop = \"alfalfa\"]) / 160000) 1
17
1
11
...
531
Set-aside
precision (100 * (count patches with [crop = \"set-aside\"]) / 160000) 1
17
1
11
...
581
Rye
precision (100 * (count patches with [crop = \"rye\"]) / 160000) 1
17
1
11
...
631
Triticale
precision (100 * (count patches with [crop = \"triticale\"]) / 160000) 1
17
1
11
...
681
Silphie
precision (100 * (count patches with [crop = \"silphie\"]) / 160000) 1
17
1
11
...
731
Miscanthus
precision (100 * (count patches with [crop = \"miscanthus\"]) / 160000) 1
17
1
11
...
781
Grass-clover ley
precision (100 * (count patches with [crop = \"grass-clover ley\"]) / 160000) 1
17
1
11
